...
/**
 * Paper/Purpur/Pufferfish build. Uses Adventure {@link net.kyori.adventure.text.Component}
 * messaging and {@link io.papermc.paper.event.player.AsyncChatEvent}. Compiled against the
 * Paper API's stable public surface (introduced at 1.19), which Paper keeps source- and
 * ABI-compatible going forward, so this single jar keeps running unmodified from 1.19.4
 * through the latest snapshot. Runtime Minecraft-version detection below is a compatibility
 * guard and diagnostic, not a recompile point.
 */
public final class SMPSpectate extends JavaPlugin {
    ...
    private ServerSoftware detectServerSoftware() {
        if (classExists("org.pufferfish.pufferfish.PufferfishConfig")
                || classExists("gg.pufferfish.pufferfish.PufferfishConfig")) {
     ...
     * warning above; no behavior branches on it because the Paper API surface this
     * plugin uses has been stable across the whole 1.19.4+ range.
     */
    ...
    private void sendPluginMessage(Player player, String rawMessage) {
        PaperMessenger.send(player, rawMessage);
    }
    ...
    private static final class PaperMessenger {
        static void send(Player player, String rawMessage) {
